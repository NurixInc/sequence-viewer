...
var Sequence = (function () {
  function Sequence(sequence, isoformName) {
    ...
    this.render = function (divId, document, options) {
      ...
      sequenceOptions.showLineNumbers =
        options.showLineNumbers === undefined ? true : options.showLineNumbers;
      sequenceOptions.wrapAminoAcids =
        options.wrapAminoAcids === undefined ? true : options.wrapAminoAcids;
      sequenceOptions.sequenceMaxHeight =
        options.sequenceMaxHeight === undefined
          ? "400px"
          : options.sequenceMaxHeight;
      sequenceOptions.title =
        options.title === undefined ? "Protein Sequence" : options.title;
      sequenceOptions.charsPerLine =
        options.charsPerLine === undefined ? 30 : options.charsPerLine;
      sequenceOptions.search =
        options.search === undefined ? false : options.search;
      sequenceOptions.toolbar =
        options.toolbar === undefined ? false : options.toolbar;
      sequenceOptions.badge =
        options.badge === undefined ? true : options.badge;
      sequenceOptions.fasta =
        options.fasta === undefined ? true : options.fasta;
      sequenceOptions.apiURL =
        options.apiURL === undefined ? true : options.apiURL;
      sequenceOptions.blast =
        options.blast === undefined ? true : options.blast;
      sequenceOptions.digestion =
        options.digestion === undefined ? false : options.digestion;

      sequenceOptions.header = options.header
        ? {
            display:
              options.header.display === undefined
                ? true
                : options.header.display,
            searchInTitle:
              options.header.searchInTitle === undefined
                ? true
                : options.header.searchInTitle,
            unit:
              options.header.unit === undefined ? "Char" : options.header.unit,
            showCpl:
              options.header.showCpl === undefined
                ? true
                : options.header.showCpl,
            badgeWithUnit:
              options.header.badgeWithUnit === undefined
                ? false
                : options.header.badgeWithUnit,
          }
        : {
            display: true,
            searchInTitle: true,
      ...
      var badge =
        ...
        '<span class="badge" style="border-radius:70%;border: 2px solid black;color:#C50063;padding:8px 5px;background-color:white;margin-right:10px;vertical-align:middle;min-width:32px;">' +
        sequence.length +
        "</span>" +
        "</div>";
      ...
      var badgeWithUnit =
        '<div style="display:inline-block;">' +
        '<span class="badge" style="border-radius:70%;border: 2px solid black;color:#C50063;padding:5px 5px;background-color:white;margin-right:10px;vertical-align:middle;min-width:32px;font-size:11px;">' +
        sequence.length +
        "<div style='margin-top:-2px;font-size:9px;color:black;text-transform:lowercase;'>" +
        sequenceOptions.header.unit +
        "</div></span>" +
        "</div>";

      var displayBadge = sequenceOptions.badge
        ? sequenceOptions.header.badgeWithUnit
          ? badgeWithUnit
          : badge
        : "";

      var header = sequenceOptions.header.display
        ? '<div class="sequenceHeader row" style="border-bottom: 1px solid #E7EAEC;padding-bottom:5px;margin:0px 0px 10px">' +
          displayBadge +
      ...
      var sources =
        ...
        '<div class="scroller" style="max-height:' +
        sequenceOptions.sequenceMaxHeight +
        ';overflow:auto;white-space: nowrap;padding-right:20px;margin-right:10px;">' +
        ...
        '<div class="fastaSeq" display-option="' +
        sequenceOptions.charsPerLine +
        '" style="font-family: monospace;font-size: 13px;display:inline-block;padding:5px;">' +
        sequence +
        "</div></div>" +
        ...
        if (isoName !== "") {
          if (sequenceOptions.fasta) {
            ...
            $(divID + " .sequenceToolbar").append(
              ...
              '<a class="btn btn-default btn-sm fasta-link" href="' +
                apiURL +
                "/isoform/" +
                isoName +
                ".fasta\" target='_blank'>View FASTA</a>"
          ...
          if (sequenceOptions.blast) {
            $(divID + " .sequenceToolbar").append(
              '<a class="btn btn-default btn-sm" href="/blast/' +
                isoName +
                '" style="margin-left:5px;">Blast sequence</a>' +
                '<a id="selectionBlast" class="btn btn-default btn-sm" href="/blast/' +
                isoName +
                '" style="margin-left:5px;"> Blast selection</a>'
            );
          }
          ...
          if (sequenceOptions.digestion) {
            $(divID + " .sequenceToolbar").append(
              '<a class="btn btn-default btn-sm" href="/tools/protein-digestion/' +
                isoName +
                '" style="margin-left:5px;"> Protein Digestion </a>'
            );
          }
        }
      }
    ...
    this.selection = function (start, end, color, options) {
      ...
      positions[0] =
        positions[0] +
        ~~(positions[0] / 10) +
        4 * ~~(positions[0] / sequenceOptions.charsPerLine);
      positions[1] =
        positions[1] +
        ~~(positions[1] / 10) +
        4 * ~~(positions[1] / sequenceOptions.charsPerLine);
      var highlightColor = color;
      hlSeq =
        hlSeq.substring(0, positions[0]) +
        "<span class='stringSelected' style=\"background:" +
        color +
        ';color:white;">' +
    ...
    function multiHighlighting(ArrayHL, color, options) {
      ...
      for (i in ArrayHL) {
        ...
        seqTemp =
          seqTemp.substring(0, positionStart) +
          "<span class='stringsSelected' style=\"background:" +
          color +
          ';color:white;">' +
    ...
    this.addLegend = function (hashLegend) {
      for (var i = 0; i < hashLegend.length; i++) {
        if (hashLegend[i].underscore === true) {
          $(divID + " .coverageLegend").append(
            '<div style="display:inline-block;background:' +
              hashLegend[i].color +
              ';width:20px;height:20px;vertical-align:middle;margin:0px 5px 0px 10px;border-radius:50%; border: 1px solid grey;text-align:center; line-height:0.8;">_</div><p style="display:inline-block;font-weight:bold;font-size:11px;font-style:italic;margin:0;padding-top:3px;vertical-align:top;">' +
              hashLegend[i].name +
              "</p></div>"
          );
        } else {
          $(divID + " .coverageLegend").append(
            '<div style="display:inline-block;background:' +
              hashLegend[i].color +
              ';width:20px;height:20px;vertical-align:middle;margin:0px 5px 0px 10px;border-radius:50%;"></div><p style="display:inline-block;font-weight:bold;font-size:11px;font-style:italic;margin:0;padding-top:3px;vertical-align:top;">' +
              hashLegend[i].name +
              "</p>"
          );
        }
    ...
    function fillGap(list) {
      ...
      for (var i = 0; i <= list.length - 1; i++) {
        ...
        if (i !== 0 && list[i - 1].end > list[i].start) {
          console.warn(
            "WARNING (error): Some positions in the coverage list are overlapping"
          );
        }
      }
      ...
      for (var i = 0; i < HashAA.length; i++) {
        ...
        if (end) {
          if (
            ...
            source +=
              seqInit.substring(
                jTranslation(HashAA[i].start),
                jTranslation(start)
              ) +
              '<span class="peptideHighlighted" style="background:' +
              highlightColor +
              ';">' +
              seqInit.substring(jTranslation(start), jTranslation(end + 1));

            source +=
              "</span>" +
              seqInit.substring(
                jTranslation(end + 1),
                jTranslation(HashAA[i].end)
              ) +
              "</span>";
            ...
            source +=
              ...
              '</span><span class="peptideHighlighted" style="background:' +
              highlightColor +
              ';">' +
              pre +
              seqInit.substring(
    ...
    function addSequenceSearch(inHeader) {
      if (inHeader) {
        $(divID + " .sequenceHeader").append(
          '<input class="inputSearchSeq form-control pull-right" type="text" style="width:40%;margin-top: 3px;" placeholder="Search in sequence.. (Regex supported)">'
        );
      } else {
        $(divID + " .sequenceBody").prepend(
          '<input class="inputSearchSeq form-control" type="text" style="width:100%;margin:3px auto 10px;" placeholder="Search in sequence.. (Regex supported)">'
        );
      }
      sequenceSearch();
    }
    ...
    function getSelectedText(document) {
      ...
      var pos_end_parsed = seqText.substring(0, caretOffset).replace(/\s+/g, "")
        .length;
    ...
    function triggerMouseSelectionEvent(subseq) {
      ...
      if (self.trigger)
        self.trigger(self.events.MOUSE_SELECTION_EVENT, { selection: subseq });
    }

    function changeCharsPerLine(selection) {
      sequenceOptions.charsPerLine = selection.value;
      self.render(divID, root, sequenceOptions);
    ...
    function addToolbar() {
      ...
      var source =
        ...
        //                "<div class=\"sequenceToolbar row\" style=\"margin-bottom:15px;\">" +
        '<div class="form-group form-group-sm sequenceToolbar" style=""> ' +
        '<label class="control-label charPerLine-label ' +
        hidexsCharPerLine +
        "\" for='" +
        divID.substring(1) +
        "-cpl' style='margin-right:5px;'>" +
        sequenceOptions.header.unit +
        " per line</label>" +
        '<select class="CPLChoice form-control ' +
        hidexsCharPerLine +
        "\" id='" +
        divID.substring(1) +
        "-cpl' style='display:inline-block;width:auto;margin-right:10px;'>" +
        "<option>Select</option>" +
        "<option value=50>50</option>" +
        "<option value=60>60</option>" +
        "<option value=70>70</option>" +
